...
AFirstPersonCharacter::AFirstPersonCharacter()
{
	...
	DEPRECATED_FP_Gun = CreateDefaultSubobject<USkeletalMeshComponent>(TEXT("DEPRECATED_FP_Gun"));
	DEPRECATED_FP_Gun->SetOnlyOwnerSee(true);			// only the owning player will see this mesh
	DEPRECATED_FP_Gun->bCastDynamicShadow = false;
	DEPRECATED_FP_Gun->CastShadow = false;
	// DEPRECATED_FP_Gun->SetupAttachment(Mesh1P, TEXT("GripPoint"));
	//DEPRECATED_FP_Gun->AttachToComponent(Mesh1P, FAttachmentTransformRules::KeepRelativeTransform, TEXT("GripPoint"));
//	DEPRECATED_FP_Gun->SetupAttachment(Mesh1P);

	DEPRECATED_FP_MuzzleLocation = CreateDefaultSubobject<USceneComponent>(TEXT("MuzzleLocation"));
	DEPRECATED_FP_MuzzleLocation->SetupAttachment(DEPRECATED_FP_Gun);
	DEPRECATED_FP_MuzzleLocation->SetRelativeLocation(FVector(0.2f, 48.4f, -10.6f));

	// Default offset from the character location for projectiles to spawn
	GunOffset = FVector(100.0f, 0.0f, 10.0f);

	// Note: The ProjectileClass and the skeletal mesh/anim blueprints for Mesh1P, DEPRECATED_FP_Gun, and VR_Gun 
...
void AFirstPersonCharacter::BeginPlay()
{
	...
	*/
	...
	//Gun->AttachToComponent(Mesh1P, FAttachmentTransformRules::SnapToTargetNotIncludingScale, TEXT("GripPoint"));


	//DEPRECATED_FP_Gun->AttachToComponent(Mesh1P, FAttachmentTransformRules(EAttachmentRule::SnapToTarget, true), TEXT("GripPoint"));
...
{
	ThePlayerInputComponent = PlayerInputComponent;
	...
	if (ProjectileClass != NULL)
	{
		...
		if (World != NULL)
		{
			...
			else
			{
				...
				const FVector SpawnLocation = ((DEPRECATED_FP_MuzzleLocation != nullptr) ? DEPRECATED_FP_MuzzleLocation->GetComponentLocation() : GetActorLocation()) + SpawnRotation.RotateVector(GunOffset);
